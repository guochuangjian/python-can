...
def dev_have_attr(device, attr):
    return (
        device in zcan_dev_property
        and attr in zcan_dev_property[device]
        ...
        for chn in channels:
            if not dev_have_attr(device, "is_net_dev"):
                if self.is_fd_device:
                    if (
                        zcanlib.SetValue(
                            ip, str(chn) + "/canfd_abit_baud_rate", str(bitrate)
                        )
                        != ZCAN_STATUS_OK
                    ):
                    ...
                    data_bitrate = (
                        kwargs["data_bitrate"] if "data_bitrate" in kwargs else bitrate
                    )
                    if (
                        zcanlib.SetValue(
                            ip, str(chn) + "/canfd_dbit_baud_rate", str(data_bitrate)
                        )
                        != ZCAN_STATUS_OK
                    ):
                    ...
                    if (
                        zcanlib.SetValue(ip, str(chn) + "/baud_rate", str(bitrate))
                        != ZCAN_STATUS_OK
                    ):
                ...
                if dev_have_attr(device, "is_tcp"):
                    if (
                        ...
                        and zcanlib.SetValue(
                            ip,
                            str(chn) + "/work_mode",
                            "1" if kwargs["is_server"] else "0",
                        )
                        != ZCAN_STATUS_OK
                    ):
                        raise ValueError(
                            "set work_mode attr failed or no work_mode attr."
                        )
                if not kwargs["is_server"] and (
                    "dst_ip" not in kwargs or "dst_port" not in kwargs
                ):
